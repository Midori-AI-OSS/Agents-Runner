...
def load_prompt(name: str, fallback: str = "", **kwargs: Any) -> str:
    ...
    Args:
        ...
        fallback: Hard-coded fallback text if file cannot be loaded
    ...
    Example:
        >>> load_prompt('headless_desktop', DISPLAY=':1', fallback='...')
    ...
    if cache_key not in _PROMPT_CACHE:
        ...
        if not prompt_file.exists():
            logger.warning(
                f"Prompt file not found: {prompt_file}, using fallback"
            )
            ...
            try:
                ...
                logger.error(
                    f"Failed to load prompt from {prompt_file}: {e}, using fallback"
                )
                _PROMPT_CACHE[cache_key] = fallback
