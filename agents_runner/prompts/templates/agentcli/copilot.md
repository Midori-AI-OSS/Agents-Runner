...
This section is appended because Copilot CLI is selected and available for this run. Use it as selection guidance and execution notes.
...
**MCP servers**
- Add/remove (always-on): edit `~/.copilot/mcp-config.json` under `mcpServers` (add an entry to enable; delete it to disable)
- Note: Copilot requires a `tools` list per server (use `["*"]` to allow all tools from that MCP server)
- Example:
  ```json
  { "mcpServers": { "playwright": { "command": "npx", "args": ["-y", "@playwright/mcp@latest"], "tools": ["*"] } } }
  ```
