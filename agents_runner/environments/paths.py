import os

from agents_runner.persistence import default_state_path

from .model import ENVIRONMENT_FILENAME_PREFIX


def default_data_dir() -> str:
    return os.path.dirname(default_state_path()) or os.getcwd()


def safe_env_id(env_id: str) -> str:
    safe = "".join(ch for ch in (env_id or "").strip() if ch.isalnum() or ch in {"-", "_"})
    return safe or "default"


def safe_task_id(task_id: str) -> str:
    """Sanitize task_id for filesystem use."""
    safe = "".join(ch for ch in (task_id or "").strip() if ch.isalnum() or ch in {"-", "_"})
    return safe or "default"


def environment_path(env_id: str, data_dir: str | None = None) -> str:
    data_dir = data_dir or default_data_dir()
    return os.path.join(data_dir, f"{ENVIRONMENT_FILENAME_PREFIX}{safe_env_id(env_id)}.json")


def managed_repos_dir(data_dir: str | None = None) -> str:
    from .task_workspaces import app_data_task_workspaces_root

    return app_data_task_workspaces_root(data_dir=data_dir)


def managed_repo_checkout_path(
    env_id: str,
    data_dir: str | None = None,
    task_id: str | None = None,
    workspace_location: object = "app_data",
) -> str:
    """
    Get the checkout path for a managed repository.

    Args:
        env_id: Environment identifier
        data_dir: Optional data directory path
        task_id: Optional task identifier for task-specific isolation

    Returns:
        Path to the checkout directory:
        - With task_id: {root}/{env_id}/tasks/{task_id}/
        - Without task_id: {root}/{env_id}/
    """
    from .task_workspaces import task_workspace_path

    return task_workspace_path(
        env_id,
        data_dir=data_dir,
        task_id=task_id,
        location=workspace_location,
    )
