...
echo "[desktop-setup] Configuring desktop environment for Docker image"

# Create base directories with proper permissions
echo "[desktop-setup] Creating base directories..."
...
echo "[desktop-setup] Discovering noVNC web root..."
NOVNC_WEB=""
...
if [ -z "${NOVNC_WEB}" ]; then
  ...
  echo "[desktop-setup] Searched: /usr/share/webapps/novnc, /usr/share/novnc, /usr/share/noVNC" >&2
  exit 1
fi
