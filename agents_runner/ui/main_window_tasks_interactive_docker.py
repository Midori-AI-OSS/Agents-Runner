...
def launch_docker_terminal_task(
    ...
    try:
        ...
        if has_rosetta() is False:
            rosetta_snippet = f'echo "[host/docker][WARN] Rosetta 2 not detected; install with: {ROSETTA_INSTALL_COMMAND}"'
...
def _prepare_preflight_scripts(
    ...
    try:
        ...
        preflight_clause += (
            ...
            'echo "[docker/preflight][INFO] system: starting"; '
            '/bin/bash "${PREFLIGHT_SYSTEM}"; '
            'echo "[docker/preflight][INFO] system: done"; '
        )
        ...
        if (settings_preflight_script or "").strip():
            ...
            preflight_clause += (
                ...
                'echo "[docker/preflight][INFO] settings: running"; '
                '/bin/bash "${PREFLIGHT_SETTINGS}"; '
                'echo "[docker/preflight][INFO] settings: done"; '
            )
        ...
        if (environment_preflight_script or "").strip():
            ...
            preflight_clause += (
                ...
                'echo "[docker/preflight][INFO] environment: running"; '
                '/bin/bash "${PREFLIGHT_ENV}"; '
                'echo "[docker/preflight][INFO] environment: done"; '
            )
        ...
        if str(extra_preflight_script or "").strip():
            ...
            preflight_clause += (
                ...
                'echo "[docker/preflight][INFO] helpme: running"; '
                '/bin/bash "${PREFLIGHT_HELP}"; '
                'echo "[docker/preflight][INFO] helpme: done"; '
            )
