...
class AgentsTabWidget(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._agent_table.setHorizontalHeaderLabels(
            ["Priority", "Agent", "ID", "Config folder", "Fallback", ""]
        )
    ...
    def _on_add_agent(self) -> None:
        ...
        self._rows.append(
            AgentInstance(agent_id=agent_id, agent_cli=agent_cli, config_dir="")
        )
        self._render_table()
    ...
    def _set_row_agent_cli(self, row_index: int, agent_cli: str) -> None:
        ...
        self._rows[row_index] = AgentInstance(
            agent_id=inst.agent_id, agent_cli=agent_cli, config_dir=inst.config_dir
        )
        self._update_fallback_options()
        self.agents_changed.emit()
    ...
    def _commit_row_agent_id(self, row_index: int, line: QLineEdit) -> None:
        ...
        self._rows[row_index] = AgentInstance(
            agent_id=new_id, agent_cli=current.agent_cli, config_dir=current.config_dir
        )
        self._render_table()
        self.agents_changed.emit()
    ...
    def _commit_row_config_dir(self, row_index: int, line: QLineEdit) -> None:
        ...
        self._rows[row_index] = AgentInstance(
            agent_id=inst.agent_id, agent_cli=inst.agent_cli, config_dir=value
        )
        self.agents_changed.emit()
    ...
    def get_agent_selection(self) -> AgentSelection | None:
        ...
        for inst in self._rows:
            ...
            if agent_id:
                agents.append(
                    AgentInstance(
                        agent_id=agent_id, agent_cli=agent_cli, config_dir=config_dir
                    )
                )
