...
## Completion
- ✅ Created `agents_runner/agent_systems/codex/` package with `__init__.py` and `plugin.py`
- ✅ Implemented `CodexPlugin` class with proper capabilities, UI theme spec, and plan method
- ✅ Plan method builds correct argv with `codex exec --sandbox danger-full-access` and conditional `--skip-git-repo-check`
- ✅ Config mount for `~/.codex` configured correctly
- ✅ Prompt delivery mode set to positional
- ✅ No Qt imports (pure Pydantic models)
- ✅ Passed ruff format and check
- ✅ Committed as `[FEAT] Add Codex agent system plugin` (850c54d)
