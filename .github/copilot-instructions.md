...
3.  **Local Rules Discovery (Required):** Before editing any file, check for `AGENTS.md` files that apply to that path (repo root + any in the directory tree). Follow the most specific instructions first, then consult `.agents/instructions/`, then repo-wide guidance.
